SELECT 'E     MORTURY' AS 'Service',
     ' ' as 'Number'
UNION ALL
SELECT 'E.1   Body Days' AS 'Service',
...
UNION ALL
SELECT 'E.2   Embalment' AS 'Service',
    IF(client_visits.visit_attribute_id IS NULL, 0, SUM(IF(client_visits.patient_gender = 'M' , 0, 0))) as 'Number' 
FROM
  (SELECT DISTINCT 
                   visit_attribute.visit_attribute_id as visit_attribute_id,
   ...
   WHERE visit_attribute.voided = 0 ) AS client_visits
UNION ALL
SELECT 'E.3   Post-mortem' AS 'Service',
    IF(client_visits.visit_attribute_id IS NULL, 0, SUM(IF(client_visits.patient_gender = 'M' , 0, 0))) as 'Number' 
FROM
  (SELECT DISTINCT 
                   visit_attribute.visit_attribute_id as visit_attribute_id,
   ...
   WHERE visit_attribute.voided = 0 ) AS client_visits
UNION ALL
SELECT 'E.4   Unclaimed Body Days' AS 'Service',
    IF(client_visits.visit_attribute_id IS NULL, 0, SUM(IF(client_visits.patient_gender = 'M' , 0, 0))) as 'Number' 
FROM
  (SELECT DISTINCT 
                   visit_attribute.visit_attribute_id as visit_attribute_id,
