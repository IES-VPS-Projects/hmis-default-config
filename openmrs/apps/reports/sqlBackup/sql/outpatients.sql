...
SELECT 'A.1.5 Total General Outpatient' AS 'Age Group',
       ...
       IF(client_visits.patient_id IS NULL, 0, SUM(IF(DATE(client_visits.first_visit_date) < DATE(client_visits.visit_date),IF(client_visits.patient_gender = 'F', 1, 1),0))) AS 'RE-ATT',
       IF(client_visits.patient_id IS NULL, 0, SUM(IF(client_visits.patient_gender = 'M' , 1, 1))) as 'TOTAL'
FROM
  (SELECT DISTINCT patient.patient_id AS patient_id,
                   observed_age_group.name AS age_group,
                   observed_age_group.id as age_group_id,
...
FROM
  (SELECT DISTINCT obs.obs_id AS obs_id,
                   ...
                   patient.date_created AS first_visit_date
   FROM form_concepts_map
     INNER JOIN obs ON obs.concept_id = 3982 OR obs.concept_id=3648 OR obs.concept_id=2923 OR obs.concept_id=4213 OR obs.concept_id=2979 OR obs.concept_id=2134
     INNER JOIN encounter ON encounter.encounter_id=obs.encounter_id
     INNER JOIN visit ON visit.visit_id=encounter.visit_id  AND DATE(visit.date_stopped) BETWEEN CAST('#startDate#' AS DATE) AND CAST('#endDate#' AS DATE) AND visit.voided = 0
     INNER JOIN patient ON patient.patient_id=visit.patient_id AND patient.voided = 0
...
SELECT 'A.6     Total Outpatient Services' AS 'Age Group',
       IF(client_visits.patient_id IS NULL, 0, SUM(IF(client_visits.patient_gender = 'F', 1, 1))) as 'NEW',
       0 AS 'RE-ATT',
       0 AS 'TOTAL'
FROM
  (SELECT DISTINCT patient.patient_id AS patient_id,
